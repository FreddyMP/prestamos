...
def create_db(nombre):
    try:
        ...
        cursor['cursor'].execute("CREATE TABLE Prestamos (id INT AUTO_INCREMENT PRIMARY KEY,id_cliente INT NOT NULL, valor_de_prestamo FLOAT(10) NOT NULL, balance_actual FLOAT(10) NOT NULL, fecha_desembolso DATE NULL, dia_de_pago INT(2) NULL, frecuencia_de_interes INT(10) NOT NULL, estado VARCHAR(12) NOT NULL default 'Abierto', aprobacion VARCHAR(12) NOT NULL default 'Pendiente', tasa_de_interes FLOAT(2) NOT NULL, fecha_cambio_tasa_de_interes DATE,  tipo_de_amortizacion VARCHAR(50) NOT NULL,  tiempo_de_prestamo INT(11) NOT NULL, aplica_garantia INT(1), fecha_creacion DATE DEFAULT CURRENT_TIMESTAMP NOT NULL,  creado_por INT(11) NOT NULL, fecha_modificacion DATE, modificado_por INT(11), fecha_eliminacion DATE, eliminado_por INT(11))")
